...
describe('LibrarySection', () => {
  ...
  it('renders four library cards and details for the first entry', () => {
    ...
    expect(
      within(detailsPanel).getByRole('heading', { name: 'Studium przypadku: izolacja rodzinna' })
    ).toBeInTheDocument();
    expect(
      within(detailsPanel).getByText(
        'Rodzina zgłosiła serię subtelnych, lecz narastających aktów izolacji i dewaluacji ze strony głowy rodziny.'
      )
    ).toBeInTheDocument();
    expect(screen.getByRole('tab', { name: 'Streszczenie' })).toHaveAttribute(
      'aria-selected',
      'true'
    );
  });
  ...
  it('allows switching entries and tabs while keeping focus on the detail panel', () => {
    ...
    expect(
      within(detailsPanel).getByRole('heading', { name: 'Analiza Kalendarza: Kronika Eskalacji' })
    ).toBeInTheDocument();
    ...
    expect(
      screen.getByText('Powtarza się cykl: prezent – milczenie – krytyka.')
    ).toBeInTheDocument();
